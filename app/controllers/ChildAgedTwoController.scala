...
class ChildAgedTwoController @Inject()(appConfig: FrontendAppConfig,
                                         ...
                                         formProvider: ChildAgedTwoFormProvider) extends FrontendController with I18nSupport {
  ...
  def onPageLoad(mode: Mode) = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      Ok(childAgedTwo(appConfig, preparedForm, mode))
  }
    ...
    implicit request =>
      form.bindFromRequest().fold(
        (formWithErrors: Form[_]) =>
          Future.successful(BadRequest(childAgedTwo(appConfig, formWithErrors, mode))),
        (value) =>
          dataCacheConnector.save[Boolean](request.internalId, ChildAgedTwoId.toString, value).map(cacheMap =>
