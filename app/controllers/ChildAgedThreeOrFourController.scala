...
class ChildAgedThreeOrFourController @Inject()(appConfig: FrontendAppConfig,
                                         ...
                                         formProvider: ChildAgedThreeOrFourFormProvider) extends FrontendController with I18nSupport {
  ...
  def onPageLoad(mode: Mode) = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      Ok(childAgedThreeOrFour(appConfig, preparedForm, mode))
  }
    ...
    implicit request =>
      form.bindFromRequest().fold(
        (formWithErrors: Form[_]) =>
          Future.successful(BadRequest(childAgedThreeOrFour(appConfig, formWithErrors, mode))),
        (value) =>
          dataCacheConnector.save[Boolean](request.internalId, ChildAgedThreeOrFourId.toString, value).map(cacheMap =>
